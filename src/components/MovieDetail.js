...
export default class MovieDetail extends Component {
    ...
    componentDidMount() {
        ...
        axios.get(`${APIURL}${id}?api_key=${APIKEY}&language=en-US`)
            .then((response) => {
        ...
        axios.get(`${APIURL}${id}/casts?api_key=${APIKEY}`)
            .then((response) => {
                this.setState({
        ...
        axios.get(`${APIURL}${id}/similar?api_key=${APIKEY}&language=en-US&page=1`)
            .then((response) => {
                this.setState({
            ...
            <div>
                <div className="App-content-wrapper">
                    <div className="Movie-wrapper">
                        <img className="movie-backdrop" src={`https://image.tmdb.org/t/p/w1280${this.state.movie_detail.backdrop_path}`} style={this.movieBackdropStyles} />


                        <div className="movie-content">
                            <div>
                                <img className="movie-poster" src={`https://image.tmdb.org/t/p/w500${this.state.movie_detail.poster_path}`} alt={`${this.state.movie_detail.original_title}`} />
                            </div>
                                ...
                                <div className="movie-actions">
                                    <div className="movie-actions__item">
                                        <span className="movie-action-circle">
                                            <a href="/login">
                                                <svg className="movie__action action__favorite" viewBox="0 0 13 12" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <div className="movie-actions__item">
                                        <span className="movie-action-circle">
                                            <svg width="10" height="15" className="movie__action action__playtrailer" viewBox="0 0 10 15" xmlns="http://www.w3.org/2000/svg"><path d="M.013.135L9.7 7.5.012 14.865">
                                            </path>
                                            </svg>
                                        </span>
                                    </div>
                                    <div className="movie-actions__item">
                                        <span className="movie-action-circle">
                                            <a href="/login"><svg className="movie__action action__watchlater" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <Cast id={this.state.movie_detail.id}
                            cast_array={this.state.cast_array}
                        />
                        <h1 className="movie-title">Similar Movies</h1>
                        <SimilarMoviesContainer id={this.state.movie_detail.id}
